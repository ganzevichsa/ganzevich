        ...
        <div class="container opened" data-animation-in="fadeInLeft" data-animation-out="fadeOutLeft">
            ...
            <header class="header">
                ...
                <div class="top-menu">
                    <ul>
                        <li class="active">
                            <a href="/people/{{$people->id}}/albumvideo/{{$videoname->id}}">
                ...
                <div class="profile no-photo" style="align-items: baseline; justify-content: flex-start; height: auto;min-height: auto;}">
                    ...
                    <div class="social" >
                        @foreach($album as $itemAlbum)
                            <a target="" href="/people/{{$people->id}}/albumvideo/{{$itemAlbum->id}}/edit" style="display: grid; float: left">
            ...
            <div class="card-inner active" id="about-card">
                <div class="card-wrap" style="">
                    ...
                    -->
                    <!-- title -->
                    <div class="title"><span class="first-letter">Редактировать видео в альбоме {{$videoname->name}}</span></div>
                    <label>Добавить видео</label>
                    <form action="{{route('video.album.edit', $people->id)}}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="media d-sm-flex align-items-center">

                            <div class="media-body pl-sm-3 text-center text-sm-left">
                                <select class="form-control" name="category" id="exampleFormControlSelect1">
                                    <option value="{{$videoname->id}}">{{$videoname->name}}</option>
                                </select>
                    ...
                    <table style="font-size: 15px" class="table table-striped table-centered mb-0">
                        ...
                        @foreach($video as $itemVideo)
                            <tr>
                                <td class="table-user" style="width: 30%;">
                                    <video src="{{ asset('images/video') }}/{{$itemVideo->url}}" alt="" class="mr-2 rounded-circle" style="width: 80%;"></video>
                                </td>
                                <form action="{{route('editnamevideo', $itemVideo->id)}}" method="POST" style="width: 90%; margin: 0 auto;">
                                    @csrf
                                    <td><input name="id" style="display: none" value="{{$itemVideo->id}}"><input name="name" class="inputalbumname" value="{{$itemVideo->name}}"></td>
                                    <td style="display: none"><button type="submit" class="action-icon inputnamesave"> <i class="icon ion-checkmark"></i></button></td>
                                </form>

                                <td class="table-action">
                                    <a href="/people/{{$itemVideo->people_id}}/{{$itemVideo->album_id}}/video/delete/{{$itemVideo->id}}" class="action-icon"> <i class="icon ion-android-delete"></i></a>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
